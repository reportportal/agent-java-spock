...
import com.epam.reportportal.spock.features.ignore.TestOneFeatureIgnore;
...
public class TestIgnoreFeatureIntegrity {
	...
	public void verify_ignored_feature_correct_reporting() {
		Result result = runClasses(TestOneFeatureIgnore.class);
		...
		assertThat(stepTypes, containsInAnyOrder(ItemType.STEP.name(), ItemType.STEP.name()));
		...
		List<FinishTestItemRQ> ignoredFeatures = finishCaptor.getAllValues()
				...
				.collect(Collectors.toList());
		assertThat(ignoredFeatures, hasSize(1));

		FinishTestItemRQ ignoredFeature = ignoredFeatures.get(0);
		assertThat(ignoredFeature.getIssue(), nullValue());
		assertThat(ignoredFeature.getEndTime(), notNullValue());
